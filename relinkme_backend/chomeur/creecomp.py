...
for comp_data in competences_data:
    competence, created_flag = Competence.objects.get_or_create(
    ...
    if created_flag:
        created += 1
        print(f"✅ Créé: {competence.libelle} ({competence.categorie})")
    else:
        existing += 1

print(f"\n📊 Résumé:")
print(f"   ✅ {created} nouvelles compétences créées")
print(f"   ℹ️  {existing} compétences existaient déjà")
print(f"   📈 Total: {Competence.objects.count()} compétences dans la base")